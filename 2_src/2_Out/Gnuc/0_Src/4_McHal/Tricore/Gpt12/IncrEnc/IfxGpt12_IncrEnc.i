# 1 "0_Src/4_McHal/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.c"
# 1 "D:\\GitRepo\\InfineonRacer\\2_src\\InfineonRacer_TC23xA//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/4_McHal/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.c"
# 29 "0_Src/4_McHal/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.c"
...
# 45 "./0_Src/4_McHal/Tricore/Gpt12/Std/IfxGpt12.h" 2
...
# 46 "./0_Src/4_McHal/Tricore/Gpt12/Std/IfxGpt12.h" 2
...
# 47 "./0_Src/4_McHal/Tricore/Gpt12/Std/IfxGpt12.h" 2
...
# 30 "0_Src/4_McHal/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.c" 2
# 40 "0_Src/4_McHal/Tricore/Gpt12/IncrEnc/IfxGpt12_IncrEnc.c"
...
{
    return ((float32)driver->turn + (float32)driver->rawPosition / (float32)driver->resolution) * 2.0 * (3.1415926535897932384626433832795);
}
...
boolean IfxGpt12_IncrEnc_init(IfxGpt12_IncrEnc *driver, const IfxGpt12_IncrEnc_Config *config)
{
    ...
    driver->positionConst = 1.0 / (float32)driver->resolution * 2.0 * (3.1415926535897932384626433832795);
    ...
    IfxGpt12_initTxInPin(config->pinA, config->pinMode);
    IfxGpt12_initTxEudInPin(config->pinB, config->pinMode);

    if (config->pinZ != ((void *)0))
    {
        IfxGpt12_initTxInPin(config->pinZ, config->pinMode);
    }

    driver->speedConstTimeDiff =
        (2.0 * (3.1415926535897932384626433832795)) / (config->base.resolution * 2) * IfxGpt12_T5_getFrequency(gpt12);
    ...
    if (config->base.speedFilterEnabled)
    {
        ...
        Ifx_LowPassPt1_init(&driver->speedLpf, &lpfConfig);
    }
...
void IfxGpt12_IncrEnc_initConfig(IfxGpt12_IncrEnc_Config *config, Ifx_GPT12 *gpt12)
{
    ...
    config->base.minSpeed = 1.0 / 60.0 * (2 * (3.1415926535897932384626433832795));
    config->base.maxSpeed = 20000.0 / 60.0 * (2 * (3.1415926535897932384626433832795));
    config->base.speedFilterEnabled = 1;
    config->base.speedFilerCutOffFrequency = config->base.maxSpeed / 2 * (3.1415926535897932384626433832795) * 10;
...
void IfxGpt12_IncrEnc_setRefreshPeriod(IfxGpt12_IncrEnc *driver, float32 updatePeriod)
{
    ...
    driver->speedConstPulseCount = (2.0 * (3.1415926535897932384626433832795)) / driver->resolution / updatePeriod;
    driver->speedModeThresholdTick = driver->speedModeThreshold * driver->resolution * updatePeriod / (2.0 * (3.1415926535897932384626433832795));
}
...
static void IfxGpt12_IncrEnc_updateSpeedFromT2(IfxGpt12_IncrEnc *driver, sint32 newPosition)
{
    ...
    {
        driver->speed = Ifx_LowPassPt1_do(&driver->speedLpf, speed);
    }
    else
    {
        driver->speed = speed;

...
{	
	...
	GROUP :
	{
		.data_emem (DATA) LOAD(> pfls0) COPYTABLE :
		{
			*(.data_emem)
		}
	...
	GROUP : 
	{			
		.data_lmu (DATA) LOAD(> pfls0) COPYTABLE :
		{
			*(.lmudata)
		}
		...
		.sdata4 (DATA) LOAD(> pfls0) COPYTABLE :
		{
			*(.sdata_a9)
			*(.lmu_data)
		}
		...
		__A9_MEM = SIZEOF(.sdata4) ? ADDR(.sdata4) + 32k : SIZEOF(.sbss4) ? ADDR(.sbss4) + 32k : (ADDR(.sdata4) & 0xF0000000) + 32k ;
	} > lmuram

# 1 "0_Src/2_CDrv/Tricore/Tft/touch.c"
# 1 "D:\\GitRepo\\InfineonRacer\\2_src\\InfineonRacer_TC23xA//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/2_CDrv/Tricore/Tft/touch.c"
# 27 "0_Src/2_CDrv/Tricore/Tft/touch.c"
...
# 28 "0_Src/2_CDrv/Tricore/Tft/touch.c" 2
...
# 29 "0_Src/2_CDrv/Tricore/Tft/touch.c" 2
...
# 30 "0_Src/2_CDrv/Tricore/Tft/touch.c" 2
# 57 "0_Src/2_CDrv/Tricore/Tft/touch.c"
typedef struct
{
    uint8 spiTxBuffer[5];
    uint8 spiRxBuffer[5];
...
# 114 "0_Src/2_CDrv/Tricore/Tft/touch.c"
...
#pragma section
# 148 "0_Src/2_CDrv/Tricore/Tft/touch.c"
void touch_init (void)
{

    boolean interruptState = IfxCpu_disableInterrupts();

    g_Qspi_Touch.drivers.spiMaster = qspi0_init();
...

    {
        ...
        const IfxQspi_SpiMaster_Output slsOutput = {&IfxQspi0_SLSO9_P20_3_OUT,
 ...
 g_Qspi_Touch.qspiBuffer.spiTxBuffer[0] = 0;
    ...
    touch_driver.xmax = 3700.0f;
    touch_driver.xmin = 240.0f;
    touch_driver.ymax = 300.0f;
    touch_driver.ymin = 3740.0f;


    touch_dasinfo.event = 0;
    touch_dasinfo.xmax = 3800.0f;
    touch_dasinfo.xmin = 340.0f;
    touch_dasinfo.ymax = 140.0f;
    touch_dasinfo.ymin = 3600.0f;
...
{
    touch_event.xdisp =
        ((float) touch_event.x - touch_driver.xmin) / (touch_driver.xmax - touch_driver.xmin) * (float) (320/8);
    if (touch_event.xdisp < 0)
        touch_event.xdisp = 0;
    if (touch_event.xdisp >= (320/8))
        touch_event.xdisp = (320/8) - 1;
    touch_event.ydisp =
        ((float) touch_event.y - touch_driver.ymin) / (touch_driver.ymax - touch_driver.ymin) * (float) (240/12);
    if (touch_event.ydisp < 0)
        touch_event.ydisp = 0;
    if (touch_event.ydisp >= (240/12))
        touch_event.ydisp = (240/12) - 1;
...
void touch_periodic (void)
{
    ...
    {
        if (IfxPort_getPinState(IfxPort_P20_0.port, IfxPort_P20_0.pinIndex) == 0)
        {
            touch_driver.xmax = 3700.0f;
            touch_driver.xmin = 240.0f;
            touch_driver.ymax = 300.0f;
            ...
            IfxQspi_SpiMaster_exchange(&g_Qspi_Touch.drivers.spiMasterChannel, &g_Qspi_Touch.qspiBuffer.spiTxBuffer[0],
                &g_Qspi_Touch.qspiBuffer.spiRxBuffer[0], 5);


            while (IfxQspi_SpiMaster_getStatus(&g_Qspi_Touch.drivers.spiMasterChannel) == SpiIf_Status_busy) {};
            touch_driver.prev_time = touch_driver.time;
            touch_driver.time = ((__extension__({ unsigned __res; __asm__ volatile ("mfcr %0, LO:" "0xFC04" : "=d" (__res) :: "memory"); __res; })) & 0x7FFFFFFF) >> 8;
    ...
    else
    {
        ...
        {
            touch_driver.xmax = 3800.0f;
            touch_driver.xmin = 340.0f;
            touch_driver.ymax = 140.0f;
            touch_driver.ymin = 3600.0f;
            touch_driver.prev_time = touch_driver.time;
            touch_driver.time = ((__extension__({ unsigned __res; __asm__ volatile ("mfcr %0, LO:" "0xFC04" : "=d" (__res) :: "memory"); __res; })) & 0x7FFFFFFF) >> 8;
            touch_driver.prev_x = touch_driver.x;
